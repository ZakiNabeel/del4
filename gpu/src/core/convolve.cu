 ...
 * 4. Write results (already in correct orientation)
   ...
   _KLT_FloatImage imgout)
 {
  ...
  // NOTE: No sync here - let caller decide when to sync
  // Stream ensures ordering: kernel → memcpy will execute in order
 ...
 * Keep data on GPU for both passes - only 2 CPU↔GPU transfers total!
   ...
   _KLT_FloatImage imgout)
 {
  ...
  // NOTE: No sync here - let caller decide when to sync
  // This allows overlapping compute/transfer when called multiple times
   ...
   _KLT_FloatImage grady)
 {
  ...
  // SYNC REQUIRED: This is a public API function - caller will immediately access
  // gradx->data and grady->data on CPU. Must ensure all GPU work is complete.
 ...
 // SYNC REQUIRED: Public API function - caller will access smooth->data on CPU
 CUDA_CHECK(cudaStreamSynchronize(g_gpu.stream));
}
  ...
  float **d_grady_out)
{
  ...
  // SYNC REQUIRED: Even though we return device pointers, caller may launch
  // kernels on default stream or access data. Ensure gradients are computed.
  CUDA_CHECK(cudaStreamSynchronize(g_gpu.stream));
}
