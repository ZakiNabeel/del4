/**********************************************************************
Finds the 150 best features in an image and traks them through the 
...
#endif
{
  ...
  FILE *fp = popen(cmd, "r");
  if (fscanf(fp, "%d", &nFrames) != 1) {
    nFrames = 10;  // Default fallback
  }
  pclose(fp);

  printf("🚀 GPU Time: Starting KLT feature tracking...\n");
  
  // Record start event (captures GPU work start time)
  // Use default stream (0) to capture all GPU work
  ...
  // Record end event and synchronize to ensure all GPU work is complete
  // Use default stream (0) to capture all GPU work
  ...
  printf("⏱️  GPU Time: Total processing time: %.3f seconds (%.2f ms)\n", 
         gpu_time_ms / 1000.0f, gpu_time_ms);
  printf("⏱️  GPU Time: Per-frame average: %.2f ms\n", gpu_time_ms / nFrames);
